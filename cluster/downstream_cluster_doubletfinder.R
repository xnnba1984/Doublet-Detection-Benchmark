...
ggplot(result, aes(x=pred.num, y=cluster)) + geom_line() + xlab('Doublets Prediction Number') + ylab('Cluster Number') + 
  ggtitle('Cluster Number over Doublets Prediction Number - doubletfinder')
...
#   xlab('Dim 1') + ylab('Dim 2') + labs(title = 'tSNE Plot of Clustering after Cleaning - doubletfinder') +
#   scale_color_manual(breaks = c("Doublet", "Singlet"), values=c("red", "gray60")) + 
#   scale_size_manual(values=c(0.7,0.3))
# ggsave("figure/doubletfinder_clean_cluster_4.png")
