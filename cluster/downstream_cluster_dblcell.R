...
sim.doublet <- sim.data[["4"]][[1]][[5]]; dim(sim.doublet)
sim.label <- sim.data[["4"]][[2]][[5]]; table(sim.label)
sim.label <- ifelse(sim.label=='doublet', 1, 0); table(sim.label)
sim.type <- sim.data[["4"]][[2]][[5]]; table(sim.type)
...
###########################################################
score.list <- readRDS("paper_result/dblcells_type.rds")
score <- score.list[[3]][[3]][[5]]
pred.num <- seq(from = 0, by = 0.005, to = 0.25) * 2000; pred.num
set.seed(10)
cluster <- pbsapply(pred.num, function(x){
  ...
  DimPlot(clean.seurat, reduction = "tsne", label = F)
...
ggplot(result, aes(x=pred.num, y=cluster)) + geom_line() + xlab('Doublets Prediction Number') + ylab('Cluster Number') + 
  ggtitle('Cluster Number over Doublets Prediction Number - Doubletcells')
saveRDS(result, "paper_result/dblcells_cluster_4.rds")

#############################################################
# cluster quality
#############################################################
result <- readRDS("paper_result/dblcells_cluster_8.rds")
cluster <- 8
...
saveRDS(error, "paper_result/dblcells_purity_8.rds")
...
ggplot(tsne, aes(x = tSNE_1, y = tSNE_2)) +
  ...
  xlab('Dim 1') + ylab('Dim 2') + labs(title = 'tSNE Plot of Clustering after Cleaning - dblcell') +
  scale_color_manual(breaks = c("Doublet", "Singlet"), values=c("red", "gray60")) + 
  scale_size_manual(values=c(0.7,0.3))
ggsave("figure/dblcell_clean_cluster_4.png")
