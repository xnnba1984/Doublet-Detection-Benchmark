...
# analyze result
labels <- sim.data[[2]][[1]]; table(labels)
labels <- ifelse(labels > 0, 1, 0); table(labels)
fg <- score1[labels == 1]
bg <- score1[labels == 0]
...
score2.list <- pblapply(list(data1, data2), function(x){
  x <- x[apply(x, 1, function(x) !all(x==0)), ]; dim(x)
  clf <- doubletdetection$BoostClassifier(n_iters=5L, use_phenograph=FALSE, standard_scaling=TRUE, random_state = 3L)
  fit <- clf$fit(t(x))
  p.values <- abs(fit$all_log_p_values_)
  score <- apply(p.values, 2, mean)
})
score2 <- abs(unlist(score2.list))
...
score4.list <- pblapply(data.chunk, function(x){
  x <- x[apply(x, 1, function(x) !all(x==0)), ]; dim(x)
  clf <- doubletdetection$BoostClassifier(n_iters=4L, use_phenograph=FALSE, standard_scaling=TRUE, random_state = 3L)
  fit <- clf$fit(t(x))
  p.values <- abs(fit$all_log_p_values_)
  score <- apply(p.values, 2, mean)
})
score4 <- abs(unlist(score4.list, use.names = F))
...
score6 <- abs(unlist(score6.list, use.names = F))
...
score8 <- abs(unlist(score8.list, use.names = F))
...
score10 <- abs(unlist(score10.list, use.names = F))
...
# save all scores
system.time(saveRDS(score.list, "sim_result/doubletdetection_distribute.rds"))
