...
saveRDS(result, "paper_result/hybrid_cluster_dbscan_8.rds")

##########################################################################################################################
# proportion of singlets in correctly identified clusters (cluster quality)
##########################################################################################################################
# 4, 6, 8 clusters
result <- readRDS("paper_result/hybrid_cluster_dbscan_8.rds")
cluster <- 8
...
# save result, change cell type, cxds or bcds accordingly
colnames(error) <- correct$pred.num; error
saveRDS(error, "paper_result/hybrid_purity_dbscan_8.rds")
